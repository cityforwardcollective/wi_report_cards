...
full_table |> 
  ...
  mutate(
    district = ifelse(broad_agency_type == "Private", glue("{district}"), district),
    ...
    "Report Card Link" = case_when(
      rc_n == "both" ~ glue(
        # use \" because some linkes have school name with apostrophe
        # e.g. Saint Paul's
    "<a href=\"{`All Students`}\" target='_blank'>All Students</a><br>",
    "<a href=\"{`Choice Students`}\" target='_blank'>Choice Students</a>"
                            ),
                rc_n == "all_only" ~ glue(
                      "<a href=\"{`All Students`}\" target='_blank'>All Students</a>",
                ),
                rc_n == "choice_only" ~ glue(
                      "<a href=\"{`Choice Students`}\"",
                            "target='_blank'>Choice Students</a>"
                ),
                            TRUE ~ "ERROR")) |> 
